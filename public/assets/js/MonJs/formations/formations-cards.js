...
function checkFormationReservationStatus(formationId) {
    console.log(`Vérification du statut des réservations pour la formation ${formationId}`);

    const timestamp = new Date().getTime();
    const url = `/check-reservation-status/${formationId}?_=${timestamp}`;
    ...
    return fetch(url)
        ...
        .then(data => {
            console.log(`Résultat de la vérification des réservations pour formation ${formationId}:`, data);
            return {
                hasConfirmedReservation: Boolean(data.has_confirmed_reservation),
                hasPendingReservation: Boolean(data.has_pending_reservation),
...
//     // 1. Vérifier d'abord si la formation est terminée
...
//     // 2. Vérifier le statut des réservations
//     checkFormationReservationStatus(formationId)
//         .then(reservationStatus => {
//             const { hasConfirmedReservation, hasPendingReservation } = reservationStatus;
            
//             if (hasConfirmedReservation) {
//                 // Cas 1: Réservations confirmées (status=1)
...
//                 // Cas 2: Réservations en attente (status=0)
//                 console.log('Formation avec réservation en attente - affichage du modal d\'avertissement');
//                 showPendingReservationModal(formationId);
//             } else {
//                 // Cas 3: Aucune réservation
//                 console.log('Aucune réservation - redirection vers la page de modification');
...
// Modal pour les réservations confirmées (status=1)
...
//                             <i class="icon-alert-triangle me-2"></i>Formation avec réservations confirmées
//                         </h5>
...
//                             <p class="mb-3">Cette formation a des réservations confirmées. La modifier pourrait affecter les participants inscrits.</p>
//                             <p class="text-muted small">Voulez-vous vraiment continuer la modification ?</p>
//                         </div>
//                     </div>
...
// Modal pour les réservations en attente (status=0)
...
// <i class="icon-clock me-2" style="color: white;"></i><span style="color: white;">Formation avec réservations en attente</span>                        </h5>
...
//                             <p class="mb-3">Cette formation a des réservations en attente de confirmation.</p>
...
// Fonction pour procéder avec la modification après confirmation - VERSION CORRIGÉE
// function proceedWithEdit(formationId) {
//     console.log(`Début de proceedWithEdit pour formation ${formationId}`);
    
//     try {
//         // Méthode 1: Fermer les modals spécifiques par ID
//         const confirmedModal = document.getElementById('confirmedReservationModal');
//         const pendingModal = document.getElementById('pendingReservationModal');
        
//         if (confirmedModal) {
//             const modalInstance = bootstrap.Modal.getInstance(confirmedModal);
//             if (modalInstance) {
//                 console.log('Fermeture du modal confirmé');
//                 modalInstance.hide();
//             }
//         }
        
//         if (pendingModal) {
//             const modalInstance = bootstrap.Modal.getInstance(pendingModal);
//             if (modalInstance) {
//                 console.log('Fermeture du modal en attente');
//                 modalInstance.hide();
//             }
//         }

//         // Méthode 2: Fermer tous les modals ouverts (backup)
...
//         // Attendre un court délai avant la redirection pour s'assurer que les modals sont fermés
//         setTimeout(() => {
//             console.log(`Redirection vers /formation/${formationId}/edit`);
            
//             // Vérifier si la redirection fonctionne
//             const redirectUrl = `/formation/${formationId}/edit`;
//             console.log(`URL de redirection: ${redirectUrl}`);
            
//             // Utiliser window.location.href avec vérification
//             try {
...
// Fonction utilitaire pour nettoyer tous les modals et backdrops
...
//         console.log('Suppression backdrop:', backdrop);
//         backdrop.remove();
//     });
    ...
    checkFormationReservationStatus(formationId)
        .then(reservationStatus => {
            ...
            if (hasConfirmedReservation && hasPendingReservation) {
                // Nouveau cas: Les deux types de réservations
                console.log('Formation avec réservations confirmées ET en attente - affichage du modal combiné');
                showCombinedReservationModal(formationId);
            } else if (hasConfirmedReservation) {
                // Cas 1: Réservations confirmées uniquement (status=1)
                console.log('Formation avec réservation confirmée uniquement - affichage du modal de confirmation');
                showConfirmedReservationModal(formationId);
            } else if (hasPendingReservation) {
                // Cas 2: Réservations en attente uniquement (status=0)
                console.log('Formation avec réservation en attente uniquement - affichage du modal d\'avertissement');
                showPendingReservationModal(formationId);
            } else {
                // Cas 3: Aucune réservation
                console.log('Aucune réservation - redirection vers la page de modification');
...
// Nouveau modal pour le cas combiné (réservations confirmées ET en attente)
function showCombinedReservationModal(formationId) {
    // Supprimer les modals existants s'ils existent
    const existingModal = document.getElementById('combinedReservationModal');
    if (existingModal) {
        existingModal.remove();
    }

    const modalHtml = `
        <div class="modal fade" id="combinedReservationModal" tabindex="-1" aria-labelledby="combinedReservationModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header bg-danger text-white">
                        <h5 class="modal-title" id="combinedReservationModalLabel">
                            <i class="icon-alert-triangle me-2"></i>Formation avec réservations multiples
                        </h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="text-center">
                            <i class="icon-users text-danger mb-3" style="font-size: 3rem;"></i>
                            <h6 class="mb-3">Attention importante !</h6>
                            <div class="alert alert-danger mb-3">
                                <strong><i class="icon-check-circle me-1"></i>Réservations confirmées</strong>
                                <p class="mb-0 small">Des participants sont déjà inscrits et confirmés pour cette formation.</p>
                            </div>
                            <div class="alert alert-warning mb-3">
                                <strong><i class="icon-clock me-1"></i>Réservations en attente</strong>
                                <p class="mb-0 small">Des demandes d'inscription sont en cours de traitement.</p>
                            </div>
                            <p class="mb-3">Cette formation a à la fois des <strong>réservations confirmées</strong> et des <strong>réservations en attente</strong>.</p>
                            <p class="text-muted small">La modifier pourrait affecter tous les participants concernés. Voulez-vous vraiment continuer ?</p>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
    ...
    document.body.insertAdjacentHTML('beforeend', modalHtml);
    const modal = new bootstrap.Modal(document.getElementById('combinedReservationModal'));
    modal.show();

    // Nettoyer le modal après fermeture
    document.getElementById('combinedReservationModal').addEventListener('hidden.bs.modal', function () {
        this.remove();
    });
}

// Modal pour les réservations confirmées uniquement (status=1)
function showConfirmedReservationModal(formationId) {
    // Supprimer les modals existants s'ils existent
    const existingModal = document.getElementById('confirmedReservationModal');
    if (existingModal) {
...
// Fonction pour procéder avec la modification après confirmation - VERSION AMÉLIORÉE
function proceedWithEdit(formationId) {
    console.log(`Début de proceedWithEdit pour formation ${formationId}`);
...
// Version alternative avec nettoyage forcé
function proceedWithEditAlternative(formationId) {
    console.log(`Version alternative de proceedWithEdit pour formation ${formationId}`);
    
    // Nettoyage forcé de tous les modals
    cleanupModals();
    
    // Redirection immédiate
    setTimeout(() => {
        const redirectUrl = `/formation/${formationId}/edit`;
        console.log(`Redirection alternative vers: ${redirectUrl}`);
        window.location.href = redirectUrl;
    }, 100);
}

// Modal pour formation terminée (conservé de votre code existant)
function showFormationEndedPopup() {
    // Supprimer les modals existants s'ils existent
    const existingModal = document.getElementById('formationEndedModal');
    if (existingModal) {
        existingModal.remove();
    }
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header bg-secondary text-white">
                        <h5 class="modal-title" id="formationEndedModalLabel">
                            <i class="icon-calendar me-2"></i>Formation terminée
                        </h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="text-center">
                            <i class="icon-calendar text-secondary mb-3" style="font-size: 3rem;"></i>
                            <h6 class="mb-3">Formation terminée</h6>
                            <p class="mb-3">Cette formation est terminée et ne peut plus être modifiée.</p>
                            <p class="text-muted small">La date de fin est dépassée.</p>
                        </div>
                    </div>
    ...
    // Nettoyer le modal après fermeture
    document.getElementById('formationEndedModal').addEventListener('hidden.bs.modal', function () {
        this.remove();
    });
}

// Fonction utilitaire pour vérifier si la formation est terminée (à conserver de votre code existant)
function isFormationEnded(endDate) {
    const today = new Date();
    const formationEndDate = new Date(endDate);
    return formationEndDate < today;
}

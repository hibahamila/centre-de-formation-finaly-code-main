...
// Vérifie l'état des réservations de l'utilisateur (en attente ou confirmées).
...
//Fournit les données du panier pour l'utilisateur authentifié.
public function getCartData()
{
    $userId = Auth::id();
    if (!$userId) {
...
// Fournit la disponibilité (places restantes) pour une liste d'IDs de formations.
public function getFormationsAvailability(Request $request)
{
    $formationIds = $request->input('formation_ids', []);
...
//Vérifie si une formation est déjà dans le panier
...
//Fournit les détails complets des formations dans le panier
